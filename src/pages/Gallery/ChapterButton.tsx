...
export const ChapterButton: React.FC<ChapterButtonProps> = ({ index, selected, wordCount, onClick }) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-xs font-medium text-gray-600 dark:text-white dark:text-opacity-60">単語数: {wordCount}</p>
      {chapterStatus !== null && (
        <>
          {chapterStatus.exerciseCount > 0 ? (
            <>
              <p className="text-xs font-medium text-gray-600 dark:text-white dark:text-opacity-60">
                練習回数: {chapterStatus.exerciseCount}
              </p>
              <p className="text-xs font-medium text-gray-600 dark:text-white dark:text-opacity-60">
                平均誤答数: {chapterStatus.avgWrongCount}
              </p>
            </>
          ) : (
            <>
              <p className="pt-1 text-xs font-medium text-gray-600 dark:text-white dark:text-opacity-60">未練習</p>
            </>
          )}
        </>
      )}
